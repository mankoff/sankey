...
#+begin_src jupyter-python :exports both
import xarray as xr
ds = xr.open_dataset("~/data/Groh_2021/AIS_GMB_basin.nc")
# ds['dm'] = ds['dm'] * ds['area']
# ds['sigma_dm'] = ds['sigma_dm'] * ds['area']
# print(ds)

ds = ds[['dm','sigma_dm']].sum(dim='reg')
df = ds.to_dataframe() / 1E12

_ = df['dm'].plot()
df = df.loc['2010-01-01':'2019-12-31']

df['dm'] = df['dm'] - df['dm'].max()
df = df.resample('YS').mean()

# print(df.mean())
# arr = unumpy.uarray(df['dm'].values, df['sigma_dm'].values)
print(df['dm'].diff().mean(), '+-', df['sigma_dm'].mean())
...
#+RESULTS:
:RESULTS:
: -339.9240282777777 +- 453.25699999999995
[[file:./figs_tmp/3764088beb65c4e1826b117472fdf8c72222342b.png]]
:END:
