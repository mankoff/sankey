...
\begin{tikzpicture}[remember picture, overlay, shift={(5.5,-5.5)}]
  ...
  \begin{sankeydiagram}
    ...
    \sankeynode{name=MELT, quantity=\Q{MELT}, at={[xshift=-4mm, yshift=1.5cm]DYN.right}, anchor=left}
    ...
    \node[anchor=center, align=center, shift=({-2.5cm,-1.5cm})] at (RFZ-from-MELT.center) {Refreezing\\(\sankeygetnodeqty{RFZ-from-MELT})};
