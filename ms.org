...
Grounding line retreat, by definition here only occurring under ice shelves, has no complete published estimates in the dimensions needed here, mass or length\textsuperscript{3} and time (e.g., (Gt or km^{3}) yr^{-1}), and are typically reported in dimensions of length and time (e.g., m yr^{-1}).
