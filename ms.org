...
We calculate mass loss and gain from the sum of all outputs minus inputs. In the Sankey diagrams we label mass loss as `drawdown or retreat', and mass gain as `build-up or expansion'.
...
Calving and frontal melt uncertainty is 30 % between those two, but combine is 10 % constrained by the upstream discharge term (see Discussion > Uncertainty).
...
A comparison of our mass change and uncertainty results with mass change and uncertainty from the independent GMB method citep:groh_2019 shows an agreement between the two within the GMB uncertainty. GMB uncertainty is 10 % in Greenland, and 115 %, 10 %, 35 %, and 30 % for Antarctic regions East, West, Peninsula, and overall - well within the uncertainty values we derive above (Table \ref{tab:results_mc}).
...
In Table \ref{tab:aq} we round to 1, with the exception of values less than 0.5 and greater than 0 which are rounded to 1. Everywhere else value are rounded to the nearest 5 Gt yr^{-1}, with the exception of values less than 2.5 and greater than 0 which are rounded up to 5.
