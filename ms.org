...
Finally, the input-output (IO) method is a hybrid of regional climate models (RCM) and observation based (velocity, thickness). It estimates inputs (e.g., snowfall) and outputs (e.g., surface melt and iceberg calving), and is typically reported at low spatial resolution - at most individual glacier basin, but in theory can be applied at the resolution of the RCMs. Temporal resolution is ~10 days or whenever a new velocity map is generated, but given the relative stability of the ice discharge (10 day resolution) to the SMB (daily), the IO method can provide some estimate of daily mass balance citet:mankoff_2021. The IO method is the only one that captures all of the processes contributing to changes of grounded ice and provides gross values and constituent terms.

Here we use a combination of the IO method over grounded ice, the altimetry method over ice shelves, and other individual remote sensing estimates of ice shelf and glacier front advance and retreat. We provide results both in traditional tabular form, and using Sankey diagrams that provide a visual method to compare magnitude of all processes within and between ice sheets, and flows representing relationships between processes. 
...
We generally avoid the term `flux' which is by definition mass or volume flow rate per unit area. Because we do not report results per unit area, we use `mass flow rate' [Gt yr^{-1}] which is equal to `volume flow rate' [km^3 yr^{-1}] assuming freshwater density of 1000 kg m^{-3}.
...
Below we detail the source of each mass flow term. We begin with the outputs as these are generally of broader interest, followed by the inputs. We then describe how net mass loss or gain are computed, and where uncertainty estimates come from. Finally, we address other methods such as regional separation, temporal alignment, and rounding.
...
Greenlandic discharge from the flux gates is split into either calving or submarine melting at the grounding line. This split is highly uncertain and minimally studied, but citet:rignot_2010 estimate that 20 - 80 % of the summer ice-front is directly melted by the ocean for the three glaciers they studied. From this, we split the discharge 50 \pm30 % between calving and submarine melt.
...
In Antarctica, the fastest rates of ice shelf grounding line retreat are in Amundsen Sea Embayment, contributing 45 Gt yr^{-1} citep:davison_2023. Although grounding line retreat occurs elsewhere in West Antarctica (e.g., at Ferrigno and Venable ice shelves), we ignore these contributions. citet:konrad_2018 showed that 85 % of all Antarctic ice sheet grounding line retreat (in terms of area change per year, from 2010 to 2016) occurs in West Antarctica. We therefore use 45 Gt yr^{-1} for Antarctica and West Antarctica, and a low estimate of 1 Gt yr^{-1} for East Antarctica and the Peninsula. This value is wrong, but with no additional information we use it as placeholder until such time as there is a better estimate of this value.

*** Grounded basal melting

Grounded basal melting citep:karlsson_2021 comes from geothermal heat flux, frictional heat from sliding, and in Greenland, viscous dissipation of surface runoff routed to the bed citep:mankoff_2017_VHD. Antarctic grounded basal melting citep:van-liefferinge_2013 excludes surface runoff.
...
Uncertainty on most individual term is provided by the upstream data products and presented in the "Unc." column on Tables \ref{tab:gl} and \ref{tab:aq}. The exceptions are calving and frontal melting in Greenland, and the derived mass loss and gain terms.

For calving and frontal melting uncertainty in Greenland see that methods section.

For derived mass loss and mass gain uncertainty we apply the independently assessed uncertainty from the GMB method citep:dohne_2023. We use Greenland, Antarctica, and Antarctic region GMB-derived mass change from 2010 through 2019 and  GMB-derived uncertainty. Recall that the GMB method only measures grounded ice. We therefore calculate the uncertainty as a percentage of GMB-estimated grounded mass loss, and then apply that uncertainty percentage to all of our components per region.

For example, if GMB uncertainty for grounded ice mass change in Antarctica is 30 %, we apply a 30 % uncertainty to both the grounded and marine mass change reported here for Antarctica. See also the Results and Discussion section.
...
# Uncertainty is 10 % in Greenland and 30 % in Antarctica.

#+NAME:tab:results_fw
#+CAPTION: Net freshwater export mass flow rate for Greenland, Antarctica, and Antarctic regions in Gt yr^{-1} and Sverdrup (1E6 m^3 s^{-1}).\label{tab:results_fw}
| Region              | Gt yr^{-1} |    Sv |
|---------------------+------------+-------|
...
Greenland lost 255 \pm25 Gt yr^{-1} from grounded ice and an additional 75 \pm10 Gt yr^{-1} (30 %) from floating ice.

Antarctica lost 190 \pm60 Gt yr^{-1} from grounded ice and an additional 260 \pm80 Gt yr^{-1} (~2.4x) from floating ice. The grounded ice mass loss is partitioned with 250 \pm25 Gt yr^{-1} lost from West Antarctica and 20 \pm5 Gt yr^{-1} lost from the Peninsula offset by 80 \pm90 Gt yr^{-1} gained in East Antarctica. Marine losses are partitioned with 275 \pm30 Gt yr^{-1} lost from West Antarctica and 175 \pm60 Gt yr^{-1} lost from the Peninsula offset by 190 \pm220 Gt yr^{-1} gained in East Antarctica.

From the GMB method, net grounded mass loss is 226 \pm 15 Gt yr^{-1} (we round this up to ~10 %) in Greenland citep:dohne_2023 and 151 \pm 44 Gt yr^{-1} (~30 %) in Antarctica citep:dohne_2023. The Antarctic regions have uncertainty of 115 %, 10 %, and 35 % for the East, West, and Peninsula respectively.
...
#+NAME:tab:results_mc
#+CAPTION: Net mass change flow rate [Gt yr^{-1}] and uncertainty [%]. Grounded, marine, and total columns are estimates from this paper. GMB and uncertainty estimates come from citet:dohne_2023. \label{tab:results_mc}
| Region     | Grounded | Marine | Total |  GMB | Unc. |
|------------+----------+--------+-------+------+------|
| /          |          |        |       |    < |      |
| Greenland  |     -255 |    -75 |  -330 | -226 |   10 |
| Antarctica |     -190 |   -260 |  -450 | -151 |   30 |
| East       |       80 |    190 |   270 |   35 |  115 |
| West       |     -250 |   -275 |  -525 | -165 |   10 |
| Peninsula  |      -20 |   -175 |  -195 |  -21 |   35 |
...
Uncertainty on the derived mass loss and mass gain terms is problematic because these terms are residuals - the small difference of several large numbers, some with large uncertainty. This is one of the major weaknesses of the IO method which is primarily used here. A basic uncertainty propagation could assume each term is independent (they are not) and report the residual values of mass loss and gain as the square root of the sum of the square of the individual uncertainty estimates. This calculation is trivial and this method produces a number, but it is unreasonably large.

Instead, we use independent information on mass loss uncertainty from direct estimates of this term with the GMB method. 

There is no physical basis and mathematical connection between the mass change estimates (residual from the IO method) and the mass change uncertainty (from the GMB method). These percentage, and the decision to apply them to floating ice, are derived using the judgement of the authorship team and knowledge of other independent products that directly measure mass change and can therefore better constrain uncertainty than the IO method used here.
